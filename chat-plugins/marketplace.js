/**
 * Wi-Fi chat-plugin - edited for use in marketplace
 * Credits: Codelegend, SilverTactic, DanielCranham, bumbadadabum, Skyla (Full)
 **/
	...
	change(key, value, user) {
		...
		if (force) {
			...
			if (!this.winner) {
				...
				this.phase = 'ended';
				this.clearTimer();
				this.send('<div style="' + GIVEAWAY_DISPLAY + '"><font style="' + GIVEAWAY_TITLE_FONT + '"><strong>' + Chat.escapeHTML(this.winner.name) + '</strong> won ' + Chat.escapeHTML(this.giver.name) + '\'s giveaway for a <strong>' + Chat.escapeHTML(this.prize) + '</strong>! Congratulations!</font><br>' + '<font style="' + GIVEAWAY_SMALL_FONT + '">Correct answer(s): ' + this.answers.join(', ') + '</font></div>');
...
class LotteryGiveaway extends Giveaway {
	...
	drawLottery() {
		...
		if (force) {
			...
			this.phase = 'ended';
			this.send('<div style="' + GIVEAWAY_DISPLAY + '"><font style="' + GIVEAWAY_TITLE_FONT + '">Lottery Draw</font><br><font style="' + GIVEAWAY_FLAVOR_FONT + '">' + Object.keys(this.joined).length + " users joined " + Chat.escapeHTML(this.giver.name) + "'s giveaway for: <strong>" + Chat.escapeHTML(this.prize) + "</strong><br/>" + '<font style="' + GIVEAWAY_SMALL_FONT + '">Our lucky winner' + (this.winners.length > 1 ? "s" : "") + ": <strong>" + Chat.escapeHTML(this.winners.reduce((prev, cur, index, array) => prev + cur.name + (index === array.length - 1 ? "" : ', '), '')) + "!</strong> Congratulations!</font>");
			...
			if (this.giver.connected) this.giver.popup("The following users have won your lottery giveaway:\n" + Chat.escapeHTML(this.winners.reduce((prev, cur, index, array) => prev + cur.name + (index === array.length - 1 ? "" : ', '), '')));
		}
		delete this.room.giveaway;
	}
}
